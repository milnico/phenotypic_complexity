...
def apply_spring_force(t: ti.i32,n_springs: ti.i32):
    for i in range(n_springs):
        #print("SPRING",i,t, spring_anchor_a[i],spring_anchor_b[i])
        a = spring_anchor_a[i]
        b = spring_anchor_b[i]
        pos_a = x[t, a]
        pos_b = x[t, b]
...
gui = ti.GUI("Mass Spring Robot", (1024, 512), background_color=0xFFFFFF)
...
def forward(output=None, visualize=True):
    ...
    for t in range(1, total_steps):
        ...
        if (t + 1) % interval == 0 and visualize:
            gui.line(begin=(0, 0.3+ground_height),
                     end=(1, 0.3+ground_height),
                     color=0x0,
                     radius=3)

            def circle(x, y, color):
                gui.circle((x, y), ti.rgb_to_hex(color), 1)

            for i in range(n_springs):

                def get_pt(x):
                    return (x[0]*scale,  (0.3+x[1]))
...
def setup_robot(objects, springs):
    ...
    for i in range(n_objects):
        o = objects[i]
        if i <=4:
            o += ((np.random.random(size=(2,))*0.02)-0.01)
        #print(o)
        #print(o+((np.random.random(size=(2,))*0.02)-0.01))
        #input("pau")
        x[0, i][0] = np.copy(o[0])
        x[0, i][1] = np.copy(o[1])
...
def evolve(seed=1):
    ...
    for i in range(epochs):
        ...
        for id in range(pop_size):
            ...
            rob = copy(population[id])
            children.append(copy(rob))
            #rob.mutate_morph(mut_rate)
            fit = 0
            for j in range(episodes):
                setup_robot(rob.objects, rob.springs)
                episode_fit = forward(visualize=False)  # optimize(toi=True, visualize=True)
                fit += episode_fit
                #print("Episode",episode_fit,id,j)
                #input("pause")
                clear()

            fit/=episodes
            print("IND FIT",id, fit)
            tmp_fit.append(fit)
            ...
            fit /= episodes
            print("Children FIT", fit)
            tmp_fit.append(fit)


        fit_index = np.argsort(tmp_fit)

        fitness = tmp_fit[fit_index[-1]]

        population = np.asarray(children)[fit_index[-pop_size:]]


        trace_fit.append(fitness)
        if fitness>absolute_fit:
            absolute_best = copy(population[-1])
            absolute_fit = fitness
            np.save("fitS" + str(seed), trace_fit)
            with open('morhpS' + str(seed), 'wb') as file:
                pickle.dump(absolute_best, file)
        #mut_rate = tmp_rate
        print(" epoch ",i, " fitness ",fitness," mut_rate ",mut_rate, " size ",len(population[-1].springs))
        setup_robot(population[-1].objects, population[-1].springs)
        fit = forward(visualize=False)  # optimize(toi=True, visualize=True)
        print("BEST", fit)
        #input("jejj")
        #if i >=139:
