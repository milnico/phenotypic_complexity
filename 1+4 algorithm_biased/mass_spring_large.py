...
gui = ti.GUI(show_gui=False)#("Mass Spring Robot", (1024, 512), background_color=0xFFFFFF)
    ...
    for i in range(epochs):
        ...
        for id in range(pop_size-1):
            ...
            if fit >= fitness:
                ...
                tmp_fit.append(fit)
                tmp_best.append(rob)
                tmp_size.append(len(rob.springs))
        ...
        if tmp_best:
            ind = np.argmax(tmp_fit)
            best_morph = tmp_best[ind]
            fitness = tmp_fit[ind]

        trace_fit.append(fitness)
        if fitness>absolute_fit:
            absolute_best = copy(best_morph)
            absolute_fit = fitness
            with open('morhpS' + str(seed), 'wb') as file:
                pickle.dump(absolute_best, file)
        #mut_rate = tmp_rate
        print(" epoch ",i, " fitness ",fitness," mut_rate ",mut_rate, " size ",len(best_morph.springs))
